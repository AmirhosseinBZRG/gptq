...
def batched_perplexity(model, dataset, tokenizer, batch_size, stride):
    device = model.device
    encodings = tokenizer("\n\n".join(dataset["text"]), return_tensors="pt", truncation=True, max_length=model.config.n_positions)
...
if __name__ == "__main__":
    ...
    model_id = "openai-community/gpt2"  # Replace with your GPT-2 model ID
