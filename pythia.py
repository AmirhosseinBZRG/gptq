...
def batched_perplexity(model, dataset, tokenizer, batch_size, stride, confidence_level=0.95):
    device = model.device
    ...
    lls_tensor = torch.stack(lls)
    mean_ll = lls_tensor.mean()
    std_ll = lls_tensor.std()
    ...
    ci_lower = torch.exp(mean_ll - z_score * (std_ll / np.sqrt(len(lls))))
    ci_upper = torch.exp(mean_ll + z_score * (std_ll / np.sqrt(len(lls))))

    return ppl, (ci_lower, ci_upper)
...
if __name__ == "__main__":
    ...
    model_id = "EleutherAI/pythia-1"  # Replace with your Pythia model ID
    ...
    # Calculate perplexity and confidence interval
    ppl, ci = batched_perplexity(model, test, tokenizer, batch_size, stride)
    print(f"--------------{ppl=}, {ci=}-------------")
